...
if __name__ == "__main__":
    ...
    Args.add_argument("--downsample", type=int, default=4)
    ...
    test_data = oem.dataset.SKKUDataset(skku_tile_list, n_classes=args.n_classes, augm=None, testing=True, patch_size=args.patch_size, downsample=args.downsample)     
    ...
    if args.sam_mask:
        ...
        mask_generator = SamAutomaticMaskGenerator(sam, crop_n_layers=args.n_layers)
    ...
    for idx, filename in tqdm(enumerate(range(len(test_data))), total=len(test_data)):
        ...
        for y in tqdm(eval_height_samples, desc="Height", total=len(eval_height_samples)):
            for x in tqdm(eval_width_samples, desc="Width", leave=False, total=len(eval_width_samples)): 
                ...
                cv2.imwrite(f"{OUTPUT_DIR}/seg_inter/seg_inter_{y}_{x}.png", seg_inter_bgr)

                crop_margin = 10    # 조절 가능
        ...
        # segmentation 결과 출력
        mask = np.argmax(prd, axis=0)
        mask[mask == 3] = 0

        seg_rgb = oem.utils.make_rgb(mask)
        ...
        img_ori_np = ((img_ori.numpy().transpose(1, 2, 0)) * 255.0).astype(np.uint8)
        sam_masks = mask_generator.generate(img_ori_np)
        sam_end_time = time.time()

        sam_time += sam_end_time - sam_start_time
        print(f"SAM mask generation time: {sam_time:.2f}s")

        mask_visual = np.zeros_like(prd[0])
        sam_mask_total = np.zeros((ori_height, ori_width), dtype=np.uint8)
        
        for sam_mask in sam_masks: 
            masking_start_time = time.time()
            # ipdb.set_trace()
            mask_res, binary_mask, mask_cls = process_single_mask((sam_mask, mask))
            masking_time += time.time() - masking_start_time
            ...
            sam_mask_total += sam_mask["segmentation"]

        os.makedirs(f"{OUTPUT_DIR}/sam", exist_ok=True)
        cv2.imwrite(f"{OUTPUT_DIR}/sam/sam_mask.png", sam_mask_total * 255)

        prd_rgb = oem.utils.make_rgb(mask_visual)
        object_map = ((mask_visual != 0) * (mask_visual != 4)).astype(np.uint8) # 0: background, 4: road

        t0 = time.time()
        img_writer((fn, prd_rgb, object_map, res_road_mask, fout.replace('.tif', '.png'), idx))
        t1 = time.time()
