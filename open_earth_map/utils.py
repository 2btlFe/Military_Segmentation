...
def visualize_mask(img, mask, alpha=0.5):
    """_summary_

    Args:
        img (numpy array): image (H x W x 3)
        mask (numpy array): semantic label (H x W)
        alpha (float): alpha value for blending
        rgd_codes (dict): dict of class-rgd code
        grey_codes (dict): dict of label code

    Returns:
        array: image with overlayed mask
    """
    ipdb.set_trace()
    ...
    out = cv2.addWeighted(img, 1-alpha, mask, alpha, 0)
...
def sam_refinement(img, segmentations, seg_logits, num_classes, predictor, coarse_thresh=0.0, minimal_area=0,
    ...
    if down_ratio != 1:
        segmentations_down = F.interpolate(segmentations.unsqueeze(0).float(), [dh, dw], mode='nearest').squeeze(0).long()
    ...
    # coarse regions, boxes, points 뽑기
